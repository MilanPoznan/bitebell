...
export default function FaqTemplate({ title, subtitle, accordionArray, demoSectionFaq, language }) {
  ...
  function setCategories() {
    const categories = accordionArray.reduce((acc, curr) => {
      ...
      allCategories.forEach((item, index) => {
        let preparedString = index !== 0 ? item.substring(1) : item
        !acc.includes(preparedString) && acc.push(preparedString)
      })
  ...
  function handleOnFiler(e, index) {
    setCurrCategory(e.target.innerText)
  ...
  return (
    <FaqWrapper>
      <FaqContainer>
        ...
        <SelectWrapper>
          <SelectFilter onChange={(e) => setCurrCategory(e.target.value)} ref={selectRef}>
