...
export default function MobileInfiniteSlider({ testemonialsTitle, testemonialsRepeater }) {
  ...
  //Set array for infinite slider 
  useEffect(() => {
    let length = testemonialsRepeater.length
    const newArr = [testemonialsRepeater[length - 1], ...testemonialsRepeater, testemonialsRepeater[0]]
    setSliderArr(newArr)
  ...
  useEffect(() => {
    ...
    if (activeSlide === 0) {
      ...
      timer = setTimeout(() => {
        ...
        setActiveSlide(sliderArr.length - 2)
      }, 300)

    } else if (activeSlide === sliderArr.length - 1) {
      timer = setTimeout(() => {
        setHasTransition(false)
        setActiveSlide(1)
      }, 300)
  ...
  }, [activeSlide, hasTransition])


  return (
    <SliderBox>
      <h2>{testemonialsTitle}</h2>
      <svg className={`arr-left`} width="38" height="26" viewBox="0 0 38 26" fill="none" xmlns="http://www.w3.org/2000/svg"
      ...
      <svg className={`arr-right `} width="38" height="26" viewBox="0 0 38 26" fill="none" xmlns="http://www.w3.org/2000/svg"
        onClick={() => setActiveSlide(activeSlide + 1)}
      >
      ...
      <SliderWrapper >
        {testemonialsRepeater !== null && sliderArr.map((item, index) =>
          <SingleSlider
            key={index}
            left={activeSlide}
            name={item.name}
